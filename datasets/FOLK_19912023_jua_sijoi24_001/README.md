# FOLK_JAKSOT: sijoitetut

- **Identifier:** `FOLK_19912023_jua_sijoi24_001.xml`
- **DOI:** `tyokay_2012-03_2012-03-01_ain_0001`
- **Temporal coverage:** 1991-01-01 - 2024-12-31
- **Published:** 2025-12-18
- **Organisation:** Tilastokeskus
- **Variable count:** 7
- **Observation count:** —
- **Population:** Suomessa vakituisesti asunut väestö kunkin vuoden viimeisenä päivänä
- **Source:** Työ- ja elinkeinoministeriö

## Description

Aineisto on osa laajempaa jaksotietoja sisältävää valmisaineistomoduulia, joka sisältää tiedot työsuhde-, työttömyys-, työvoimakoulutus-, sijoitus-, työnhaku- ja eläkejaksoista sekä taustatiedot yrityksen toimipaikasta. Sijoitetut-valmisaineistomoduuli sisältää tietoja toimenpitein sijoitetuista henkilöistä. Aineistossa on mukana tietoja sijoitusten alku- ja loppupäivistä, sijoituksen päättämisen syistä, toimenpiteistä ja työnantajasektoreista alkaen vuodesta 1991. 

<b> Valmisaineiston tarkentava nimi </b>
Jaksotiedot työsuhde-, työttömyys-, työvoimakoulutus-, sijoitus-, työnhaku- ja eläkejaksoista, sisältäen taustatietoja yrityksen toimipaikasta. 

<b> Aineiston perusjoukko, koostaminen ja tietolähteet </b> 
Aineisto sisältyy FOLK _JAKSOTt -moduuliin. Aineiston lähteenä toimii Työ- ja elinkeinoministeriön tiedot toimenpitein sijoitetuista, jota kerätty Työssäkäyntitilastoa varten.
 
<b> Aineiston päivitysaikataulu </b>
Tutustu valmisaineistojen <a href="https://stat.fi/fi/palvelut/palvelut-tutkijoille/tutkimusaineistot/valmisaineistot/valmisaineistojen-paivitysaikataulu">  päivitysaikatauluun</a>.

<b> Aineiston käyttö ja tilaaminen </b> 
Aineisto on tarkoitettu käytettäväksi FIONA-etäpalvelun kautta, ja se on linkitettävissä muihin henkilövalmisaineistomoduuleihin suojatun henkilönumeron avulla. Tietoja voi tilata tutkimuksen kohdejoukolle ja tietylle ajanjaksolle.    

Aineiston kokonaisaineiston (kaikki muuttujat koko populaatiolle ja kaikilta saatavissa olevilta vuosilta) käyttöoikeus myönnetään vain, jos tutkimuksellinen tarve sitä erityisesti edellyttää. Aineistosta voi tilata muuttujia voi tilata myös räätälöidyn version, joka sisältää vain osan valmisaineiston muuttujista. 

<b> Lisätietoja </b> 
Lisätietoja Tilastokeskuksen tutkijapalveluista: tutkijapalvelut@stat.fi

## Variables (7)

| Identifier | Name | Unit | Classification | Group |
|---|---|---|---|---|
| `vuosi` | Vuosi | — | — | — |
| `hid_e` | Suojattu henkilön yksilöivä tunniste | — | — | — |
| `alkupvm` | Sijoituksen alkupäivämäärä | — | — | — |
| `loppupvm` | Sijoituksen loppupäivämäärä | — | — | — |
| `tpide` | Toimenpidekoodi | — | — | — |
| `psyy` | Sijoituksen päättymisen syy | — | — | — |
| `ta_sektori` | Työnantajan sektori | — | — | — |

### Variable definitions

#### `vuosi` — Vuosi

Tilastovuosi on se kalenterivuosi, jota tilasto koskee.

#### `hid_e` — Suojattu henkilön yksilöivä tunniste

Tilastokeskuksessa muodostettu suojattu henkilön yksilöivä tunniste, joka on sama kaikissa henkilövalmisaineistoissa. Yhtenäisesti suojattu hid_e -tunnus mahdollistaa henkilöä koskevien tietojen yhdistämisen eri vuosien ja aineistojen välillä.

#### `alkupvm` — Sijoituksen alkupäivämäärä

Sijoituksen alkupäivämäärä.

#### `loppupvm` — Sijoituksen loppupäivämäärä

Sijoituksen loppupäivämäärä.

#### `tpide` — Toimenpidekoodi

01 = valtion työtehtävät (voimassa 1994 saakka) 
02 = Kuntien valtionosuus (voimassa 1994 saakka) 
03 = Kuntainliittojen valtionosuus (voimassa 1994 saakka) 
05 = 18-24-v. tyött. työllistäminen (voimassa 1994 saakka) 
13 = vajaakunt. työllistämistuki (voimassa 1994 saakka) 
15 = yli 10 kk työttömien työllist. (voimassa 1994 saakka) 
16 = harjaantuminen (voimassa 1994 saakka) 
19 = yrittäjäraha (voimassa 1994 saakka) 
20 = rakennemuutostyöllistäminen (voimassa 1994 saakka) 
26 = vaikeat työttömyyskunnat (voimassa 1994 saakka) 
27 = vaikeasti työllistettävät (voimassa 1994 saakka) 
28 = omatoiminen työllistyminen (voimassa 1994 saakka) 
31 = valtion velvoitetyöllistäminen (voimassa 1994 saakka) 
32 = kuntien velvoitetyöllistäminen (voimassa 1994 saakka) 
33 = huolenpitosopimus (voimassa 1994 saakka) 
34 = velvoitesijoitus (voimassa 1994 saakka) 
35 = työllistämispalvelu (voimassa 1994 saakka) 
40 = valtion työtehtävät, 
41 = työllistämispalvelu, (voimassa 2001 saakka) Työllistäminen ELY-virastoon
42 = työviraston työohjeen mukainen sijoitus,(voimassa 2003 saakka) 
50 = kunnallinen työllistämistuki, 
51 = oppisopimussijoitus, kunta, 
53 = yhdistelmätuki, kunta, 
54 = palkkatuki TMT:lla kunta 
55 = oppisopimustoimenpide 
56 = oppisopimustoimenpide 
57 = palkkatuki nuor.kunta 
58 = oppisopimus nuor. kunta 
60 = työllistämistuki, yksityinen työnantaja, 
61 = starttiraha ei-työttömille 
62 = yrittäjäraha, 
63 = osa-aikalisälle jäänyt 
64 = harjaantuminen, 
65 = starttiraha TMT:lla 
67 = työharjoittelu vastavalmist. (voimassa 1997 saakka) 
68 = oppisopimussijoitus, yksityinen, 
69 = yhdistelmätuki, yksityinen työnantaja, 
70 = työharjoittelu työmarkkinatuella, 
71 = Työelämävalmennus työttömyysetuudella 
73 = työllistäminen osa-aikatyöhön (paikkaan välitetty, tilastoidaan), 
74 = 500 pv yksityinen työmarkkinatuki, 
75 = 500 pv kunta työmarkkinatuki 
80 = palkkatuki TMT:lla yksit. 
81 = palkkatuki nuor.yksit. 
87 = oppisopimus nuor.yksit. 
88 = oppisopimus yksit. 
89 = oppisopimus yksit. 
5T = Palkkatuki kunnalle (tyött.kesto)
5V = Palkkatuki kunnalle (vamma/sairaus)
6T = Palkkatuki yksityinen (tyött.kesto)
6V = Palkkatuki yksityinen (vamma/sairaus)

#### `psyy` — Sijoituksen päättymisen syy

0 = Työllistämisen osa päättynyt 
1 = Määräaika päättynyt 
2 = Saanut työllistämispaikasta työtä 
3 = Saanut muuta työtä 
4 = Aloittanut koulutuksen 
5 = Irtisanottu 
6 = Muu kuin 1-5 tai ei tietoa 
7 = Keskeyttänyt ja jäänyt työttömäksi   
9 = Tuntematon (käytössä 1997-2004)

#### `ta_sektori` — Työnantajan sektori

1 = Valtio 
2 = Kunta 
3 = Yksityinen työnantaja, yhteisö tms. 
4 = Yritys 
5 = Yksityishenkilö, kotitalous 
6 = Kuntayhtymä, kuntainliitot 

HUOM. Vuoden 2000 luokitus poikkeaa tästä, eikä ole tiedossa, millä luokituksella tiedot ovat tällöin tulleet.

---

[← Back to catalogue](../../README.md)
