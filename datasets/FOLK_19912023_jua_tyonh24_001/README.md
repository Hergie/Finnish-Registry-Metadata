# FOLK_JAKSOT: työnhakijat

- **Identifier:** `FOLK_19912023_jua_tyonh24_001.xml`
- **DOI:** `tyokay_2012-03_2012-03-01_ain_0001`
- **Temporal coverage:** 1991-01-01 - 2024-12-31
- **Published:** 2025-12-22
- **Organisation:** Tilastokeskus
- **Variable count:** 6
- **Observation count:** —
- **Population:** Suomessa vakituisesti asuvat, vuoden viimeisenä päivänä vähintään 15 vuotta täyttäneet henkilöt
- **Source:** Työ- ja elinkeinoministeriön työnhakijarekisteri

## Description

Aineisto on osa laajempaa jaksotietoja sisältävää valmisaineistomoduulia, joka sisältää tiedot työsuhde-, työttömyys-, työvoimakoulutus-, sijoitus-, työnhaku- ja eläkejaksoista sekä taustatiedot yrityksen toimipaikasta.

Työnhakijat–osa-aineisto sisältää tietoa työnhaun alkamis- ja päättymispäivistä, päättymisen syistä ja työnhakijoiden työllisyyskoodista vuodesta 1991 alkaen.

<b> Valmisaineiston tarkentava nimi </b>
 
Jaksotiedot työsuhde-, työttömyys-, työvoimakoulutus-, sijoitus-, työnhaku- ja eläkejaksoista, sisältäen taustatietoja yrityksen toimipaikasta. 

<b> Aineiston perusjoukko, koostaminen ja tietolähteet </b>

Sisältyy FOLK_JAKSOT -moduuliin. Aineiston lähdeaineistona toimii Työ- ja elinkeinoministeriön työnhakijarekisteri. 

<b> Aineiston päivitysaikataulu </b>

Tutustu valmisaineistojen <a href="https://stat.fi/fi/palvelut/palvelut-tutkijoille/tutkimusaineistot/valmisaineistot/valmisaineistojen-paivitysaikataulu">  päivitysaikatauluun</a>.

<b> Aineiston käyttö ja tilaaminen </b>
Aineisto on tarkoitettu käytettäväksi FIONA-etäkäyttöjärjestelmän kautta, ja se on linkitettävissä muihin henkilövalmisaineistomoduuleihin suojatun henkilönumeron avulla. Tietoja voi tilata tutkimuksen kohdejoukolle ja tietylle ajanjaksolle. 

Käyttöoikeus voidaan myöntää kokonaisaineistoon (kaikki muuttujat koko populaatiolle kaikilta saatavissa olevilta vuosilta), kun sille on tutkimuksellinen tarve. Jos tarve koskee vain osaa muuttujista, voidaan aineistosta tilata räätälöity versio.

<b> Lisätietoja </b> 
Lisätietoja Tilastokeskuksen tutkijapalveluista: tutkijapalvelut@stat.fi

## Variables (6)

| Identifier | Name | Unit | Classification | Group |
|---|---|---|---|---|
| `vuosi` | Vuosi | — | — | — |
| `hid_e` | Suojattu henkilön yksilöivä tunniste | — | — | — |
| `alkupvm` | Työnhaun alkamispäivä | — | — | Työnhakijarekisteri |
| `loppupvm` | Työnhaun päättymispäivä | — | — | Työnhakijarekisteri |
| `tyol` | Työnhakijan työllisyyskoodi | — | tyottomy_11_2012_01_01 | Työnhakijarekisteri |
| `psyy` | Työnhaun päättymissyy | — | tyottomy_3_2012_01_01 | Työnhakijarekisteri |

### Variable definitions

#### `vuosi` — Vuosi

Tilastovuosi on se kalenterivuosi, jota tilasto koskee.

#### `hid_e` — Suojattu henkilön yksilöivä tunniste

Tilastokeskuksessa muodostettu suojattu henkilön yksilöivä tunniste, joka on sama kaikissa henkilövalmisaineistoissa. Yhtenäisesti suojattu hid_e -tunnus mahdollistaa henkilöä koskevien tietojen yhdistämisen eri vuosien ja aineistojen välillä.

#### `alkupvm` — Työnhaun alkamispäivä

**Group:** Työnhakijarekisteri

Työnhaun alkamispäivä.

#### `loppupvm` — Työnhaun päättymispäivä

**Group:** Työnhakijarekisteri

Työnhaun päättymispäivä.

#### `tyol` — Työnhakijan työllisyyskoodi

**Classification:** tyottomy_11_2012_01_01 · **Group:** Työnhakijarekisteri

Työnhakijan työllisyyskoodi työnhaun alussa. Jos työnhaku on alkanut ennen vuodenvaihdetta niin työllisyyskoodi on vuoden alun mukainen tilanne. 

0 = Työllistetty (ennen käytettyjä nimiä: tukityöllistetty, toimenpitein sijoitettu) 
1 = Työssä yleisillä työmarkkinoilla
2 = Työtön 
3 = Lomautettu 
4 = Lyhennetyllä työviikolla 
5 = Työvoiman ulkopuolella 
6 = Työttömyyseläkkeellä 
7 = Työllistymistä edistävässä palvelussa (työvoiman ulkopuolella olevien alaryhmä, merkitään kuntouttavassa työtoiminnassa, valmennuksessa ja työ-/koulutuskokeilussa olevat) vuodesta 2012 alkaen
8 = Koulutuksessa (työvoiman ulkopuolella olevien alaryhmä, merkitään työvoimakoulutuksessa ja omaehtoisessa opiskelussa olevat) vuodesta 2012 alkaen
9 = Tuntematon

#### `psyy` — Työnhaun päättymissyy

**Classification:** tyottomy_3_2012_01_01 · **Group:** Työnhakijarekisteri

Työnhaun päättymissyy. 

Uusi luokitus tilastovuodesta 2012 alkaen: 

00 = Työllistetty (entinen nimi: sijoitettu toimenpitein) 
01 = Välitetty työhön yleisille työmarkkinoille 
02 = Lomautus tai lyh. työviikko päättynyt 
03 = Saanut itse työpaikan 
04 = Aloittanut työvoimakoulutuksen 
05 = Siirtynyt työvoiman ulkopuolelle (nykyisin merkitään soveltuvin osin syillä 11, 12 ja 13) 
06 = Muu syy tai ei tietoa 
07 = Siirtynyt työttömyyseläkkeelle 
08 = Aloittanut muun koulutuksen 
09 = Työhön/työnhakuun EU-/ETA-valtioon 
10 = Ei ole uusinut työnhakuaan (tuli ennen korvertoituna ’6’:ksi) 
11 = Aloittanut valmennuksen/kokeilun 
12 = Aloittanut kuntouttavan työtoiminnan 
13 = Aloittanut omaehtoisen opiskelun työttömyysetuudella 

Aiemmin: 

0 = Sijoitettu työllistämistoimenpitein 
1 = Välitetty työhön yleisille työmarkkinoille 
2 = Lomautus tai lyh. työviikko päättynyt 
3 = Saanut itse työpaikan 
4 = Aloittanut työvoimakoulutuksen 
5 = Siirtynyt työvoiman ulkopuolelle 
6 = Muu syy tai ei tietoa 
7 = Siirtynyt työttömyyseläkkeelle 
8 = Aloittanut muun koulutuksen 
9 = Siirtynyt työnhakuun ETA-valtioon

---

[← Back to catalogue](../../README.md)
